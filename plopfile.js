  ...
  plop.setGenerator('Component', {
    ...
    prompts: [
      ...
      },
      {
        ...
        default: cssExtension
      }
    ],
    actions: function (answers) {
      var actions = []
      var componentPath = 'src/components/{{subPath subpath}}{{properCase name }}'
      actions.push(function (answers) {
        process.chdir(plop.getPlopfilePath())
        var fs = require('fs')
        componentPath = plop.renderString(componentPath, answers)
        addDirectory(componentPath, fs)
        return plop.renderString('{{name}} directory is created in src/components', answers)
      })
      ...
      if (answers.container) {
        actions.push({
          type: 'add',
          path: componentPath + '/{{ properCase name }}Container.js',
      ...
      actions.push({
        type: 'add',
        path: 'src/components/{{subPath subpath}}{{ properCase name}}/{{properCase name}}.' + answers.cssExt,
